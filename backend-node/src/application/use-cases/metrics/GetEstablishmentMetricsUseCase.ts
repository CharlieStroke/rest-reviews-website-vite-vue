...
import { IMetricsRepository, EstablishmentMetricSummary } from '../../../domain/repositories/IMetricsRepository';
...
export class GetEstablishmentMetricsUseCase {
    ...
    async execute(establishmentId: string): Promise<EstablishmentMetricSummary> {
        ...
        const metrics = await this.metricsRepository.getEstablishmentSummary(establishmentId);
        if (!metrics) {
