...
import { HistoricalMetricsResponse, TimeSeriesDataPoint } from '../../dtos/MetricsDTO';
...
export class GetHistoricalMetricsUseCase {
    ...
    async execute(establishmentId: string, days: number = 30): Promise<HistoricalMetricsResponse> {
        const est = await this.establishmentRepository.findById(establishmentId);
        if (!est) {
            throw new AppError('Establishment not found', 404);
        }

        const snapshots = await this.metricsRepository.getHistoricalMetrics(establishmentId, days);
