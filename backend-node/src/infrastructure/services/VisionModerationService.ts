...
export async function checkImageSafety(
  ...
  if (!env.SIGHTENGINE_API_USER || !env.SIGHTENGINE_API_SECRET) {
    console.warn(
      "[Moderation] SIGHTENGINE credentials not set — skipping NSFW check",
    );
    return;
  }
  ...
  form.append(
    "media",
    new File([new Uint8Array(imageBuffer)], "upload", { type: mimeType }),
  );
  ...
  if (
    (nudity &&
      (nudity.raw >= NUDITY_BLOCK_THRESHOLD ||
        nudity.partial >= NUDITY_BLOCK_THRESHOLD)) ||
    (gore && gore.prob >= GORE_BLOCK_THRESHOLD)
  ) {
    throw new AppError(
      "La imagen fue rechazada por contener contenido inapropiado.",
      422,
    );
  }
}
