...
import {
  CreateEstablishmentPostSchema,
  UpdateEstablishmentPostSchema,
} from "../../../application/dtos/EstablishmentPostDTO";
import { AuthRequest } from "../middlewares/AuthMiddleware";

@injectable()
export class EstablishmentPostController {
  constructor(
    @inject(ListEstablishmentPostsUseCase)
    private listUseCase: ListEstablishmentPostsUseCase,
    @inject(CreateEstablishmentPostUseCase)
    private createUseCase: CreateEstablishmentPostUseCase,
    @inject(UpdateEstablishmentPostUseCase)
    private updateUseCase: UpdateEstablishmentPostUseCase,
    @inject(DeleteEstablishmentPostUseCase)
    private deleteUseCase: DeleteEstablishmentPostUseCase,
  ) {}
  ...
  public list = async (req: Request, res: Response): Promise<void> => {
    ...
    const { data, total } = await this.listUseCase.execute(slug, {
      page,
      limit,
    });
  ...
  public create = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    const post = await this.createUseCase.execute(slug, dto, {
      id: user.userId,
      role: user.role,
    });
  ...
  public update = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    const post = await this.updateUseCase.execute(postId, dto, {
      id: user.userId,
      role: user.role,
    });
  ...
  public delete = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    await this.deleteUseCase.execute(postId, {
      id: user.userId,
      role: user.role,
    });
